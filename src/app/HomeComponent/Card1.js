"use client"
import Link from 'next/link'


export default function Card1() {
  
        return (
        <>
            <section className='bg-white text-black pt-2'>
                <div className='container m-auto grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-1 p-1'>
                <div className='h-[275px]  md:h-[300px] lg:h-[350px] shadow-sm rounded-[4px]'>
                        <img src='https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSRUJefeY1mcZ17Uyy9NmcAd5vhuMTCrD7n_Q&s' className='h-[55%] w-full rounded-[4px]' />
                        <h2 className='text-[20px] font-[700] px-1'>Healers Hospital bijnor
                        </h2>
                        <p className='px-1 overflow-hidden text-[10px] text-[#5c5b5b] lg:text-[15px] md:text-[13px]'>
                        is aa leading multispecialty hospital located in Bijnor. Renowned for its exceptional patient-centered care
                        </p>
                        <Link href='/Hospitaldetail'>
                         <button type='submit' className='border block m-auto text-[15px] lg:text-[16px]  bg-[#0a0a0a] font-[600] text-white rounded-md lg:h-[40px] h-[25px] sm:h-[30px] mt-2 w-[95%] sm:w-[95%]' >See More </button></Link>
                      
                     
                    </div>
                    <div className='h-[275px]  md:h-[300px] lg:h-[350px] shadow-md rounded-[4px]'>
                        <img src='https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSRUJefeY1mcZ17Uyy9NmcAd5vhuMTCrD7n_Q&s' className='h-[55%] w-full rounded-[4px]' />
                        <h2 className='text-[20px] font-[700] px-1'>Healers Hospital 
                        </h2>
                        <p className='px-1 text-[10px] lg:text-[15px] md:text-[13px]'>
                        is a leading multispecialty hospital located in Bijnor. Renowned for its exceptional patient-centered care
                        </p>
                        <Link href='/Hospitaldetail'>
                         <button type='submit' className='border-2 px-6 py-2 mt-1 rounded-lg text-[#636161] block m-auto text-[16px] lg:text-[18px] [65%]' >See More </button></Link>
                      
                     
                    </div>
                    <div className='h-[275px]  md:h-[300px] lg:h-[350px] shadow-md rounded-[4px]'>
                        <img src='https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSRUJefeY1mcZ17Uyy9NmcAd5vhuMTCrD7n_Q&s' className='h-[55%] w-full rounded-[4px]' />
                        <h2 className='text-[20px] font-[700] px-1'>Healers Hospital 
                        </h2>
                        <p className='px-1 text-[10px] lg:text-[15px] md:text-[13px]'>
                        is a leading multispecialty hospital located in Bijnor. Renowned for its exceptional patient-centered care
                        </p>
                        <Link href='/Hospitaldetail'>
                         <button type='submit' className='border block m-auto text-[16px] lg:text-[18px]  bg-[#798de6] text-white rounded-md lg:h-[40px] h-[25px] sm:h-[30px] mt-2 w-[70%] sm:w-[65%]' >SEE MORE </button></Link>
                      
                     
                    </div>
                    <div className='h-[275px]  md:h-[300px] lg:h-[350px] shadow-md rounded-[4px]'>
                        <img src='https://static.vecteezy.com/system/resources/thumbnails/000/219/529/small/medical-background.jpg' className='h-[55%] w-full rounded-[4px]' />
                        <h2 className='text-[20px] font-[700] px-1'>Healers Hospital 
                        </h2>
                        <p className='px-1 text-[10px] lg:text-[15px] md:text-[13px]'>
                        is a leading multispecialty hospital located in Bijnor. Renowned for its exceptional patient-centered care
                        </p>
                        <Link href='/Hospitaldetail'>
                         <button type='submit' className='border block m-auto text-[16px] lg:text-[18px]  bg-[#2c4cdb] text-white rounded-md lg:h-[40px] h-[25px] sm:h-[30px] mt-2 w-[70%] sm:w-[65%]   '>SEE MORE </button></Link>
                      
                     
                    </div>
                    <div className='h-[275px]      md:h-[300px]  lg:h-[350px]  rounded-[4px] shadow-md'>
                        3
                    </div>
                    <div className='h-[275px]      md:h-[300px]  lg:h-[350px]  rounded-[4px] shadow-md'>
                        3
                    </div>
                    <div className='h-[275px]      md:h-[300px]  lg:h-[350px]  rounded-[4px] shadow-md'>
                        3
                    </div>
                    <div className='h-[275px]      md:h-[300px]  lg:h-[350px]  rounded-[4px] shadow-md'>
                        3
                    </div>
                    <div className='h-[275px]      md:h-[300px]  lg:h-[350px]  rounded-[4px] shadow-md'>
                        3
                    </div>
                    <div className='h-[275px]      md:h-[300px]  lg:h-[350px]  rounded-[4px] shadow-md'>
                        3
                    </div>
                    <div className='h-[275px]      md:h-[300px]  lg:h-[350px]  rounded-[4px] shadow-md'>
                        3
                    </div>
                    <div className='h-[275px]      md:h-[300px]  lg:h-[350px]  rounded-[4px] shadow-md'>
                        3
                    </div>
                    <div className='h-[275px]      md:h-[300px]  lg:h-[350px]  rounded-[4px] shadow-md'>
                        3
                    </div>
                    <div className='h-[275px]      md:h-[300px]  lg:h-[350px]  rounded-[4px] shadow-md'>
                        3
                    </div>

                    
                </div>
            </section>
        </>
    )
}
